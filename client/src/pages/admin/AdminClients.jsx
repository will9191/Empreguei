...
const AdminClients = () => {
  ...
  const fetchCompanies = async () => {
    const response = await axios.get(`${url}/client/search?page=${page}`);
  ...
  return (
    <Container>
      <TopField>
        <TopText> CANDIDATOS CADASTRADAS</TopText>
      </TopField>

      {Array.isArray(data) && Object.keys(data) ? (
        data?.map((client) => (
          <Field key={client._id}>
            <Area>
              <AreaPic>
                <Pic src={client.picture} />
              </AreaPic>
              <NameField>{client.firstName}</NameField>{' '}
